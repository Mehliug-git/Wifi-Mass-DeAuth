...
def client_grab(mac, channel):
    global client
    command = f"airodump-ng --bssid {mac} --channel {channel} {wlanmon}" # essaye de chopper le client
    output = handshake_grab(command, timeout_airodump_client)
    # Utilisation de la regex pour grep le client
    regex = rf'{mac} ([A-F0-9:]+)'
    match = re.search(regex, output)
    if match:
        client = match.group(0)
        print(BOLD, GREEN,f'BOOM ! For {mac} the Client is : {client}', RESET)
    else:
        print(RED,"[-] No clients Found ",RESET)
        print(BLUE,f'[*] Result of Client : {output} ')
    

def handshake_grab(mac, channel):
    global handshake
